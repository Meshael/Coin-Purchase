//
//  StoreKitManeger.swift
//  Coin Purchase
//
//  Created by Mashael Alharbi on 17/02/2023.
//
...
class StoreKitManager: ObservableObject {
    ...
    @Published var purchasedCoins : [Product] = []
    ...
    func updateCustomerProductStatus() async {
        ...
        for await result in Transaction.currentEntitlements {
            do {
                ...
                // since we only have one type of producttype - .nonconsumables -- check if any storeProducts matches the transaction.productID then add to the purchasedCoins
                if let course = storeProducts.first(where: { $0.id == transaction.productID}) {
                    purchasedCoins.append(course)
                }
    ...
    func isPurchased(_ product: Product) async throws -> Bool {
        //as we only have one product type grouping .nonconsumable - we check if it belongs to the purchasedCoins which ran init()
        return purchasedCoins.contains(product)
    }
}
